...
export function Landing({ handleClick }: { handleClick: () => void }) {
  return (
    <div className="bg-slate-100 rounded-lg border border-slate-200 py-4 w-7/12 m-auto">
      <div className="w-10/12 m-auto">
        ...
        />
        <h2 className="my-5 font-suse font-semibold text-justify text-2xl text-slate-800">
        ...
        <h2 className="text-justify my-5 font-suse font-light text-xl">
          No more endless scrolling—get exactly what you need in seconds.
        </h2>
      </div>
      <button
        onClick={handleClick}
        className="bg-lime-600 text-slate-200 border-slate-900 border p-2 rounded-md drop-shadow-md font-new-amsterdam text-2xl hover:bg-lime-700 hover:drop-shadow-xl">
        ENTER
      </button>
    </div>
  );
